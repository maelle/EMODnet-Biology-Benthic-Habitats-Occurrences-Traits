...
spp_attr <- biotic_aphias_matched %>%
  ...
  bind_rows()
```
```{r, echo = FALSE, message = FALSE, eval = !get_spp_attr}
...
library("pointblank")
spp_attr_agent <- create_agent(spp_attr)
spp_attr_agent <- col_vals_null(spp_attr_agent, vars(adult_2)) 
```
...
spp_attr_agent <- col_vals_null(spp_attr_agent, "larva > planula_2")
```
...
# Let's check our special test :-)
dplyr::mutate(spp_attr, ok = !(!is.na(larva) & !is.na(`larva > planula`))) %>% dplyr::pull(ok) %>% head()

spp_attr_agent <- specially(spp_attr_agent, function(x) {dplyr::mutate(x, ok = !(!is.na(larva) & !is.na(`larva > planula`))) %>% dplyr::pull(ok) })
```
...
spp_attr_agent <- specially(spp_attr_agent, function(x) {dplyr::mutate(x, ok = !(!is.na(polyp) & !is.na(adult))) %>% dplyr::pull(ok) })
interrogate(spp_attr_agent)
```
