...
```{r benthic-data-habitat-matching-1, message = FALSE}
library(tidyverse)
library(worrms)
library(raster)
library(sf)
...
```{r benthic-data-habitat-matching-2, message = FALSE, eval=FALSE}
curl::curl_download(
...
```{r benthic-data-habitat-matching-3}
...
```{r benthic-data-habitat-matching-4}
...
```{r benthic-data-habitat-matching-5}
...
```{r benthic-data-habitat-matching-6}
# I was getting Error : id must be of class numeric, integer
wm_benth_classification <- wm_classification_(id = as.numeric(pull(benth_taxa, AphiaID))) 

benth_classification <- wm_benth_classification %>%
  dplyr::select(-AphiaID) %>% 
  pivot_wider(names_from = rank, values_from = scientificname) %>% 
  unnest(cols = c(Kingdom, Phylum, Subphylum, Superclass, Class, Subclass,
...
```{r benthic-data-habitat-matching-7, message = FALSE}
curl::curl_download(
  url = "https://pureportal.strath.ac.uk/files/69962174/data_files_csv.zip",
  destfile = here::here("data/raw_data/strath_synthetic_nw_european_shelf_sed_map.zip"))

# unzip
unzip(here::here("data/raw_data/strath_synthetic_nw_european_shelf_sed_map.zip"), exdir = here::here("data/raw_data/strath_sedmap"))
# remove zip file
invisible(file.remove(
...
sed_props <- read_csv(
  here::here("data/raw_data/strath_sedmap/sediment_properties.csv"))
sed_props_descriptors <- read_csv(
  here::here("data/raw_data/strath_sedmap/sediment_properties_descriptors.csv"))
...
```{r benthic-data-habitat-matching-8, warning = FALSE}
...
```{r benthic-data-habitat-matching-12, warning = FALSE}
...
```{r benthic-data-habitat-matching-15}
# sed_by_event has a MudPercent column too!
# was this supposed to be a join?
benth_events <- benth_events %>% bind_cols(sed_by_event)
```

If you want to check where the unmatched events are:
```{r benthic-data-habitat-matching-16}
...
```{r benthic-data-habitat-matching-17}
curl::curl_download(
...
```{r benthic-data-habitat-matching-18, message = FALSE}
st_layers(
  here::here("data",
...
```{r benthic-data-habitat-matching-19}
...
```{r benthic-data-habitat-matching-20, message = FALSE, warning = FALSE}
...
```{r benthic-data-habitat-matching-21}
...
```{r benthic-data-habitat-matching-27}
...
```{r benthic-data-habitat-matching-28}
...
```{r benthic-data-habitat-matching-29}
...
```{r benthic-data-habitat-matching-30}
## datetime
Sys.time()
## repository
git2r::repository()
